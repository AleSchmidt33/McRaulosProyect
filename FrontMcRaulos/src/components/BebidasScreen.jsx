  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Tu back: { status:'OK', data: [...] } o { status:'OK', data:{ productos:[...] } }
        ...
        console.error("Fallo /productos/tipo_productos/:id (bebidas)", e);
      } finally {
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="bg-white/95 backdrop-blur rounded-3xl shadow-xl border border-white/40 p-6 mb-6 flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-800">
            {selectedType?.nombre ? `Bebidas — ${selectedType.nombre}` : "Bebidas"}
          </h1>
        ...
        {loading && <p className="text-white/90">Cargando bebidas…</p>}
        {err && (
        ...
        {!loading && !err && items.length === 0 && (
          <div className="bg-white/90 text-gray-700 border border-white/40 rounded-xl p-4 mt-6">
            No encontramos bebidas para este tipo.
          </div>
        )}
      </div>
    </div>
